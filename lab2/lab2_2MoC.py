...
def calculate_phases_and_amplitudes(coefficients):
    ...
    for A, B in coefficients:
        C = sqrt(A**2 + B**2)  # Calculate amplitude
        if A == 0:  # Handle the case when A is zero
            if B > 0:
                phi = pi / 2  # 90 degrees
            elif B < 0:
                phi = -pi / 2  # -90 degrees
            else:
                phi = 0  # A and B are both zero, phase is zero
        else:
            phi = atan(B/A)  # Calculate phase normally
...
def plot_amplitudes_and_phases(amplitudes, phases):
    ...
    plt.stem(range(len(amplitudes)), amplitudes, "r", markerfmt="ro",
             basefmt=" ", label="|C_k|")
    ...
    plt.figure(figsize=(10, 6))
    plt.stem(range(len(phases)), phases, "r", markerfmt="ro",
...
def print_table_and_formula(amplitudes, phases, Tc):
    ...
    for i in range(1, len(amplitudes) - 1):
        print(f" + 2*{amplitudes[i]:.4f}*cos({2 * i}*pi*t/Tc + {phases[i]:.4f})", end="")
    print(f" + {amplitudes[-1]}*cos({2 *(len(amplitudes) - 1)}*pi*t/Tc + {phases[-1]:.4f})")
